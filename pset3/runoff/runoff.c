...
void tabulate(void)
{
    ...
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            candidate_choice = preferences[i][j];
...
bool print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        votes_amount += candidates[i].votes;
    ...
    //checks if the the candidate has more then 50% of votes
    if (maxVote > (float)votes_amount / 2)
    {
        printf("%s\n", candidates[currentWinIndex].name);
        return true;
    }
